...
const Member = () => {
    ...
    const fetchData = async (skip, limits) => {
        try {
            setLoading(true);
            const response = await axios.get(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/all-member?skip=${skip}&limit=${limits}`, { withCredentials: true });
    ...
    const handleSearchData = async () => {
        ...
        try {
            setLoading(true);
            const response = await axios.get(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/search?searchTerm=${search}`, { withCredentials: true });
            setData(response.data.members);
