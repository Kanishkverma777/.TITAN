...
const MemberDetail = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/member-details/${id}`, { withCredentials: true });
    ...
    const handleSwitchBtn = async () => {
        ...
        try {
            await axios.put(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/change-status/${id}`, { status: newStatus }, { withCredentials: true });
            setStatus(newStatus);
    ...
    const handleDeleteMember = async () => {
        if (window.confirm("Are you sure you want to delete this member? This action cannot be undone.")) {
            try {
                const response = await axios.delete(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/delete-member/${id}`, { withCredentials: true });
    ...
    const handleRenewSaveBtn = async () => {
        ...
        try {
            const response = await axios.put(`${(process.env.REACT_APP_API_URL || 'http://localhost:4000')}/members/update-plan/${id}`, { membership: planMember }, { withCredentials: true });
            if (response.status === 200) {
                    ...
                    <div className='w-full lg:w-2/3'>
                        ...
                        <div className='grid grid-cols-2 gap-4 md:gap-8 border-y border-titan-grey/30 py-6 md:py-10 mb-6 md:mb-10'>
                            ...
                            <div className='space-y-1'>
                                ...
                                <p className='text-base md:text-2xl font-black italic'>{formatDate(data?.createdAt)}</p>
                            </div>
                            <div className='space-y-1'>
                                <p className='text-[8px] md:text-[10px] font-black text-titan-muted tracking-[0.2em] md:tracking-[0.3em]'>LAST TRANSACTION</p>
                                <p className='text-base md:text-2xl font-black italic'>{formatDate(data?.lastPayment)}</p>
                            </div>
                            <div className='space-y-1'>
                                <p className='text-[8px] md:text-[10px] font-black text-red-500 tracking-[0.2em] md:tracking-[0.3em]'>NEXT RENEWAL</p>
                                <p className='text-base md:text-2xl font-black italic text-red-500'>{formatDate(data?.nextBillDate)}</p>
                            </div>
                        </div>
