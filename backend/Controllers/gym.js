...
exports.register = async(req,res)=>{
    try{
        ...
        const isExist = await Gym.findOne({
            $or: [
                { userName: { $regex: `^${trimmedUser.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, $options: 'i' } },
                { email: trimmedEmail }
            ]
        });

        if(isExist){
            if (isExist.email === trimmedEmail) {
...
const getCookieOptions = (req) => {
    const isProduction = process.env.NODE_ENV === 'production' || (req && (req.secure || req.headers['x-forwarded-proto'] === 'https'));
    return {
        httpOnly: true,
        secure: isProduction,
...
exports.login = async(req,res)=>{
    try{
        ...
        if(gym && await bcrypt.compare(password,gym.password)){
            ...
            const token = jwt.sign({gym_id: gym._id}, jwtSecret, {expiresIn: '1d'});
...
exports.logout = async(req,res)=>{
    res.clearCookie('cookie_token', getCookieOptions(req)).json({ message: 'Logged out successfully' });
}
